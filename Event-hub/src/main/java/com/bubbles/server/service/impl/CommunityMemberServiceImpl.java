...
public class CommunityMemberServiceImpl implements CommunityMemberService {
    ...
    public void joinCommunity(Integer communityId, Integer userId) {
        ...
        if (community == null || !"ACTIVE".equals(community.getStatus())) {
            throw new BusinessException(404, "Community does not exist or has been deactivated");
        }
        ...
        if (existingMember != null && "ACTIVE".equals(existingMember.getStatus())) {
            throw new BusinessException(400, "Already joined this community");
        }
    ...
    public void leaveCommunity(Integer communityId, Integer userId) {
        ...
        if (member == null) {
            throw new BusinessException(400, "You are not a member of this community");
        }
        ...
        if ("ADMIN".equals(member.getRole())) {
            ...
            if (adminCount <= 1) {
                throw new BusinessException(400, "You are the only admin of this community. Please transfer admin rights first");
            }
        }
    ...
    public void updateMemberRole(Integer memberId, String role, Integer operatorUserId) {
        ...
        if (!"ADMIN".equals(operatorRole)) {
            throw new BusinessException(403, "No permission to modify member role");
        }
        ...
        if ("ADMIN".equals(member.getRole()) && !"ADMIN".equals(role)) {
            ...
            long adminCount = communityMemberMapper.selectCount(queryWrapper);
            if (adminCount <= 1) {
                throw new BusinessException(400, "Cannot demote the only admin. The community must have at least one admin");
            }
        }
    ...
    public void removeMember(Integer memberId, Integer operatorUserId) {
        ...
        String operatorRole = communityMemberMapper.getMemberRole(member.getCommunityId(), operatorUserId);
        if (!"ADMIN".equals(operatorRole)) {
            throw new BusinessException(403, "No permission to remove member");
        }
        ...
        if ("ADMIN".equals(member.getRole())) {
            ...
            long adminCount = communityMemberMapper.selectCount(queryWrapper);
            if (adminCount <= 1) {
                throw new BusinessException(400, "Cannot remove the only admin");
            }
        }
